...
const FactProfile = () => {
  ...
  return (
    <motion.div
      className="flex bg-white p-8 rounded-lg shadow-md mt-8"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      ...
      <div className="w-1/2">
        ...
        <ul className="list-disc pl-6">
          {facultyData.publications.map((publication, index) => (
            <li key={index}>
              {publication.title} - {publication.journal || publication.conference}, {publication.year}
            </li>
          ))}
        </ul>
      </div>

      {/* Add more sections and features based on your needs */}
    </motion.div>
  );
};
